...
{
  if ([MWMRouter isOnRoute] || [MWMRouter type] == MWMRouterTypeRuler || [self routeOptimizationEnabled] == enabled)
    return;
  routing::RoutingOptions::SaveRouteOptimizationToSettings(enabled);
  if (enabled && GetFramework().GetRoutingManager().OptimizeRoutePoints() && [MWMRouter isRoutingActive])
