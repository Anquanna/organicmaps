...
UNIT_TEST(RoutingManager_ReplaceStopWithMyPositionKeepsSlot)
{
  for (auto const type : {RouteMarkType::Start, RouteMarkType::Finish})
  {
    ...
    bool const isStart = type == RouteMarkType::Start;
    auto endpoint = MakeRoutePointOnXAxis(type, 0, isStart ? -1.0 : 10.0);
    ...
    manager.ReplaceRoutePoint(RouteMarkType::Intermediate, isStart ? 1 : 2, std::move(replacement));

    auto const expected = isStart ? std::vector<double>{5, 3} : std::vector<double>{1, 5};
    TEST_EQUAL(GetStopCoordinates(manager), expected, ());
