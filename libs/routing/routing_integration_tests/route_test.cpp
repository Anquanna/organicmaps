...
UNIT_TEST(Germany_FrankfurtDarmstadt_KeepAlternativeAfterRebuild)
{
  ...
  // Frankfurt -> Darmstadt provides distinct fastest and distance-biased routes.
  Checkpoints const checkpoints(FromLatLon(50.1109, 8.6821), FromLatLon(49.8728, 8.6512));

  // The selection survives CalculateRoute calls until ClearState; isolate shared test components.
  ...
  TEST_EQUAL(
      components.GetRouter().CalculateRoute(advanced, {} /* startDirection */, true /* adjust */, delegate, adjusted),
